...
namespace Microsoft.Band
{
    internal abstract class BandClient : IBandClient, IDisposable, IBandNotificationManager, IBandPersonalizationManager, IBandSensorManager, IBandTileManager
    {
        ...
        private static class KnownTiles
        {
            ...
            {
                Workouts, Run, Bike, Sleep, Exercise, AlarmStopwatch, UV, Weather, Finance, Starbucks,
                GuidedWorkouts, Email, Facebook, Twitter, Cortana, Lync, FBMessenger, Feed,
                Whatsapp, SMS, Calls, Calendar
            };
            public static readonly HashSet<Guid> AllTileGuids = new(AllTileIds.Select(id => Guid.Parse(id)));
        }
    }
}
