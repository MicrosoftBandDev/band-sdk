...
namespace Windows.Data.Xml.Dom
{
	public partial class XmlDocument : IXmlNode, IXmlNodeSerializer, IXmlNodeSelector
	{
		...
		private async Task SaveToFileAsync(FileInfo file)
		{
			if (file is null)
			{
			...
			await Task.Run(() => _backingDocument.Save(file.FullName));
		}

		public static async Task<XmlDocument> LoadFromFileAsync(FileInfo file)
		{
			if (file is null)
			{
				throw new ArgumentNullException(nameof(file));
			...
			await Task.Run(() => document._backingDocument.Load(file.FullName));
			return document;
		}
