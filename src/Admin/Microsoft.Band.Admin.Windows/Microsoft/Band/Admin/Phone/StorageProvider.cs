...
internal sealed class StorageProvider : IStorageProvider
{
    ...
    {
        appRootFolder = ApplicationData.get_Current().get_LocalFolder();
    ...
    internal static void CleanDeprecated()
    {
        ...
        {
            if (result[i].get_Name().Equals(text, StringComparison.OrdinalIgnoreCase))
            {
    ...
    internal static StorageProvider Create(string userId, string deviceId)
    {
        ...
        StorageFolder result = WindowsRuntimeSystemExtensions.AsTask<StorageFolder>(ApplicationData.get_Current().get_LocalFolder().CreateFolderAsync(text, (CreationCollisionOption)3)).Result;
    ...
    public Stream OpenFileForRead(StorageProviderRoot root, string relativePath, int bufferSize = 0)
    {
        ...
        if (bufferSize == -1)
        {
            bufferSize = (int)Math.Min(val.get_Size(), 8192uL);
        }
    ...
    public long GetFileSize(StorageProviderRoot root, string relativePath)
    {
        ...
        try
        {
            Logger.Log(LogLevel.Verbose, "Retrieved file size for phone file:{0} is {1}", relativePath, (long)((IRandomAccessStream)result).get_Size());
            return (long)((IRandomAccessStream)result).get_Size();
        }
        finally
        {
    ...
    public DateTime GetFileCreationTimeUtc(StorageProviderRoot root, string relativePath)
    {
        ...
        Logger.Log(LogLevel.Verbose, "Retrieved file creation UTC time for phone file:{0} is {1}", relativePath, result.get_DateCreated().UtcDateTime);
        return result.get_DateCreated().UtcDateTime;
    }
    ...
    private void CopyFolder(StorageFolder sourceFolder, StorageFolder destFolder)
    {
        ...
        {
            StorageFolder result = WindowsRuntimeSystemExtensions.AsTask<StorageFolder>(destFolder.CreateFolderAsync(item2.get_Name(), (CreationCollisionOption)3)).Result;
