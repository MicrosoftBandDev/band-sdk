...
public class WebTileResource : IWebTileResource
{
    ...
    internal List<Dictionary<string, string>> GetFeedContentMappings(XmlDocument data)
    {
        ...
        foreach (KeyValuePair<string, string> item in Content)
        {
            ...
            {
                string text = xmlNamespaceHelper.ResolveNodeWithNamespace((IXmlNode)(object)data.get_DocumentElement(), item.Value);
                if (text != null)
                {
        ...
        XmlNodeList val;
        bool isRSS;
        if (((string)data.get_DocumentElement().get_LocalName()).ToLower() == "rss")
        {
            val = data.get_DocumentElement().SelectNodes("/rss/channel/item");
            isRSS = true;
        }
        else
        {
            val = data.get_DocumentElement().SelectNodes("/feed/entry");
            isRSS = false;
        }
        int num2 = Math.Min(((IReadOnlyCollection<IXmlNode>)val).Count, 8);
        for (uint num3 = 0u; num3 < num2; num3++)
        {
            string uniqueItemId = GetUniqueItemId(xmlNamespaceHelper, isRSS, val.Item(num3));
            ...
            foreach (KeyValuePair<string, string> item3 in Content)
            {
                ...
                {
                    string text2 = null;
                    text2 = xmlNamespaceHelper.ResolveNodeWithNamespace(val.Item(num3), item3.Value);
                    if (text2 != null)
                    {
    ...
    internal void ResolveContentMappings(Dictionary<string, string> mappings, XmlDocument document)
    {
        ...
        foreach (KeyValuePair<string, string> item in Content)
        {
            string text = xmlNamespaceHelper.ResolveNodeWithNamespace((IXmlNode)(object)document.get_DocumentElement(), item.Value);
            if (text != null)
            {
