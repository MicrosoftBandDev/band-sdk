    ...
    public class WebTile : IWebTile
    {
        ...
        public async Task<bool> ResolveContentMappingsAsync()
        {
            ...
            for (int index = 0; index < webTileResourceArray.Length; ++index)
            {
                ...
                if (obj != null)
                {
                    ...
                    if (obj is XmlDocument xmlDoc)
                        iwtr.ResolveContentMappings(this.variableMappings, xmlDoc);
        ...
        internal void ReadResourceCache()
        {
            ...
            foreach (WebTileResource resource in this.Resources)
                resource.CacheInfo = webTileCacheInfo.ResourceCacheInfo[resource.Url];
        ...
        internal void WriteResourceCache()
        {
            ...
            if (Resources.Count() > 1 && this.Resources[0].Style == ResourceStyle.Simple)
        ...
        private void EnsureLastSyncInfoLoaded()
        {
            ...
            {
                Logger.LogException(LogLevel.Info, ex, $"Unexpected error reading file LastSync.json");
        ...
        private string UserSettingsFilePath
        {
            ...
            {
                this.CheckStorageProviderAndDataFolderPath();
                return Path.Combine(DataFolderPath, "UserSettings.json");
            }
        }
        ...
        public void SaveResourceAuthentication()
        {
            ...
            if (this.PrivateResources == null)
                return;
            foreach (WebTileResource privateResource in PrivateResources)
            {
                if (privateResource.Username != null && privateResource.Password != null)
                {
                    string str = privateResource.Url + TileId.ToString();
#if WINDOWS
                    vault.Add(new PasswordCredential(str, privateResource.Username, privateResource.Password));
#else
                    SecureStorage.SetAsync(str + "_username", privateResource.Username).Wait();
                    SecureStorage.SetAsync(str + "_password", privateResource.Password).Wait();
#endif
                }
                else
                {
#if WINDOWS
                    RemoveResourceCredentials(vault, privateResource);
#else
                    RemoveResourceCredential(privateResource);
#endif
                }
            }
        }
        ...
        public void LoadResourceAuthentication()
        {
            ...
            foreach (WebTileResource privateResource in PrivateResources)
            {
                string str = privateResource.Url + TileId.ToString();
#if WINDOWS
                IReadOnlyList<PasswordCredential> resourceCredentialsList = GetResourceCredentialsList(vault, privateResource);
...
#else
                PasswordCredential passwordCredential = GetResourceCredential(privateResource);
                ...
                {
                    privateResource.Username = null;
                    privateResource.Password = null;
                }
            }
        }
        ...
        {
            string str = resource.Url + TileId.ToString();
            try
            {
...
#else
        private void RemoveResourceCredential(IWebTileResource resource)
        {
            PasswordCredential resourceCredential = GetResourceCredential(resource);
            if (resourceCredential == null)
                return;
            SecureStorage.Remove(resourceCredential.Resource + "_username");
            SecureStorage.Remove(resourceCredential.Resource + "_password");
        }

        private PasswordCredential GetResourceCredential(IWebTileResource resource)
        {
            string str = resource.Url + TileId.ToString();
            try
            {
                ...
                {
                    Resource = str,
                    UserName = SecureStorage.GetAsync(str + "_username").Result,
                    Password = SecureStorage.GetAsync(str + "_password").Result,
                };
            }
            catch
            {
