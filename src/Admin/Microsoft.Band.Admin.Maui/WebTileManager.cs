...
internal class WebTileManager : IWebTileManager
{
    ...
    public async Task<IWebTile> GetWebTilePackageAsync(Stream source, string sourceFileName)
    {
        IWebTile webTile = await Task.Run(delegate
        {
            ...
            try
            {
                IWebTile webTile2 = DeserializeWebTilePackageFromJson(Path.Combine("Temp_WebTile", "manifest.json"));
                webTile2.Validate();
                return webTile2;
            }
            catch
    ...
    private void InstallWebTile(IWebTile webTile)
    {
        ...
        string text2 = Path.Combine(text, "Package");
        ...
        webTile.DataFolderPath = Path.Combine(text, "Data");
    ...
    public IWebTile GetWebTile(Guid tileId)
    {
        ...
        try
        {
            IWebTile webTile = DeserializeWebTilePackageFromJson(Path.Combine(text2, "manifest.json"));
    ...
    public async Task<string> DownloadWebTileAsync(string url, CancellationToken cancellationToken)
    {
        ...
        string localWebTileTempFileRelativePath = Path.Combine("Temp_Zip", text);
    ...
    {
        var queryParams = Flurl.Url.ParseQueryParams(uri.Query);
        if (queryParams.TryGetFirst("action", out var actionVal)
            && actionVal.ToString().Equals("download-manifest", StringComparison.OrdinalIgnoreCase))
        {
            if (queryParams.TryGetFirst("url", out var urlVal))
            {
