...
internal static class ZipUtils
{
    internal static void Unzip(IStorageProvider storageProvider, StorageProviderRoot root, Stream zipStream, string extractFolder)
    {
        ...
        Logger.Log(LogLevel.Info, $"Unzip file to folder: {extractFolder}");
        ...
        stopwatch.Stop();
        Logger.Log(LogLevel.Info, $"Time to unzip: {stopwatch.Elapsed}");
    }
    ...
    {
        Logger.Log(LogLevel.Info, $"Unzip file: {entry.FullName}");
